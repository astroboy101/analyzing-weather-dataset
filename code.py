...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregated_value=pd.pivot_table(df,columns=[date_col],aggfunc=agg)
...
#print(agg_values_ina_month(weather,'Date/Time','Temp (C)',{'Temp (C)':np.mean}))
...
mean_weather=group_values(weather,'Weather',{'Temp (C)':'mean'})
print(mean_weather)
